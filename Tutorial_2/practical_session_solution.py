"""
In this example we demonstrate hyperparameter optimization and model selection using Optuna on the Iris dataset.
KNeighbors without scaling gives the best results.!!!
"""
...
def objective(trial):
    ...
    classifier_name = trial.suggest_categorical(
        "classifier",
        ["SVC", "RandomForest", "NaiveBayes", "KNeighbors", "LogisticRegression"],
    )
    ...
    if classifier_name == "SVC":
        ...
        classifier_obj = sklearn.neighbors.KNeighborsClassifier(
            n_neighbors=knn_n_neighbors
        )
    elif classifier_name == "LogisticRegression":
        lr_c = trial.suggest_float("lr_c", 1e-10, 1e10, log=True)
        classifier_obj = sklearn.linear_model.LogisticRegression(C=lr_c, max_iter=1000)
...
if classifier_name == "SVC":
    ...
    classifier_obj = sklearn.ensemble.RandomForestClassifier(
        max_depth=best_params["rf_max_depth"], n_estimators=10
    )
elif classifier_name == "NaiveBayes":
    classifier_obj = sklearn.naive_bayes.GaussianNB()
elif classifier_name == "KNeighbors":
    ...
    classifier_obj = sklearn.linear_model.LogisticRegression(
        C=best_params["lr_c"], max_iter=1000
    )

"""
This example shows how to use Optuna to find the minimum of a function. We will use the function np.floor(np.sqrt((X - 2) ** 2 + (Y + 1) ** 2) + 3) + 0.5
"""
...
# %%
# Plot floor(sqrt((x - 2)^2 + (y + 1)^2) + 3) + 0.5
x = np.linspace(-5, 5, 500)
y = np.linspace(-5, 5, 500)
X, Y = np.meshgrid(x, y)
Z = np.floor(np.sqrt((X - 2) ** 2 + (Y + 1) ** 2) + 3) + 0.5
plt.contourf(X, Y, Z, levels=50, cmap="viridis")
plt.colorbar(label="Function Value")
plt.xlabel("x")
plt.ylabel("y")
plt.title("Contour Plot of floor(sqrt((x - 2)^2 + (y + 1)^2) + 3) + 0.5 ")
...
ax.set_ylabel("y")
ax.set_zlabel("f(x, y)")
ax.set_title("3D Surface Plot of floor(sqrt((x - 2)^2 + (y + 1)^2) + 3) + 0.5")

# Mark the minimum point
ax.scatter(2, -1, 3.5, color="red", s=100, label="Minimum (2, -1, 3.5)")
ax.legend()
...
def function_to_optimize(x, y):
    ...
    # Minimum is at (2, -1) with value 3.5
    return np.floor(np.sqrt((x - 2) ** 2 + (y + 1) ** 2) + 3) + 0.5
